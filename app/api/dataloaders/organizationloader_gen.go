...
type OrganizationLoaderConfig struct {
	...
	Fetch func(keys []string) ([]*dbmodels.Organization, []error)
...
type OrganizationLoader struct {
	...
	cache map[string]*dbmodels.Organization
...
func (l *OrganizationLoader) Load(key string) (*dbmodels.Organization, error) {
...
func (l *OrganizationLoader) LoadThunk(key string) func() (*dbmodels.Organization, error) {
	l.mu.Lock()
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() (*dbmodels.Organization, error) {
			return it, nil
		}
	}
	if l.batch == nil {
	...
	return func() (*dbmodels.Organization, error) {
		...
		var data *dbmodels.Organization
...
func (l *OrganizationLoader) LoadAll(keys []string) ([]*dbmodels.Organization, []error) {
	results := make([]func() (*dbmodels.Organization, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	organizations := make([]*dbmodels.Organization, len(keys))
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *OrganizationLoader) LoadAllThunk(keys []string) func() ([]*dbmodels.Organization, []error) {
	results := make([]func() (*dbmodels.Organization, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *OrganizationLoader) Prime(key string, value *dbmodels.Organization) bool {
	l.mu.Lock()
	var found bool
...
func (l *OrganizationLoader) unsafeSet(key string, value *dbmodels.Organization) {
	if l.cache == nil {
		l.cache = map[string]*dbmodels.Organization{}
	}
	l.cache[key] = value
}
